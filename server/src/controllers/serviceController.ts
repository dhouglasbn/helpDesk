	...
	createService = async (request: OurRequest, reply: Response) => {
		if (!request.user?.role || request.user.role !== "admin") {
			return reply
				...
				.json({
					message:
						"Acesso negado: Somente o admin pode criar serviços para os técnicos.",
				});
		}
	...
	updateService = async (request: OurRequest, reply: Response) => {
		if (!request.user?.role || request.user.role !== "admin") {
			return reply
				...
				.json({
					message:
						"Acesso negado: Somente o admin pode atualizar serviços para os técnicos.",
				});
		}
		...
		try {
			...
			const newService = await this.serviceService.updateService(
				serviceId,
				title,
				price,
			);
	...
	deactivateService = async (request: OurRequest, reply: Response) => {
		if (!request.user?.role || request.user.role !== "admin") {
			return reply
				...
				.json({
					message:
						"Acesso negado: Somente o admin pode desativar serviços dos técnicos.",
				});
		}

...
export default class UserService {
	...
	createTechAccount = async (name: string, email: string, password: string) => {
		...
		const defaultAvailabilities = ["08:00", "09:00", "10:00", "11:00", "14:00", "15:00", "16:00", "17:00"]
	...
	createClientAccount = async (name: string, email: string, password: string) => {
		const emailAleradyInUse = await db.query.users.findFirst({ where: eq(schema.users.email, email) })
		if (emailAleradyInUse) {
			throw new Error("Email already in use")
		}
	...
	updateAdminAccount = async (updatingUserId: string, { newName, newEmail, newPassword }: UpdateUserObject) => {
		const userWithNewEmail = await db.query.users.findFirst({ where: eq(schema.users.email, newEmail) })

		const userExists = await db.query.users.findFirst({ where: eq(schema.users.id, updatingUserId) })
	...
	updateTechAccount = async (updatingUserId: string, { newName, newEmail, newPassword }: UpdateUserObject) => {
		const userWithNewEmail = await db.query.users.findFirst({ where: eq(schema.users.email, newEmail) })
	...
	updateClientAccount = async (updatingUserId: string, { newName, newEmail, newPassword }: UpdateUserObject) => {
		const userWithNewEmail = await db.query.users.findFirst({ where: eq(schema.users.email, newEmail) })
	...
	updateTechAvailabilities = async (userId: string, newAvailabilities: string[]) => {
		...
		await db.delete(schema.techniciansAvailabilities).where(eq(schema.techniciansAvailabilities.userId, userId))
		...
		return await db.insert(schema.techniciansAvailabilities).values(availabilityRows).returning()
	}
	...
	deleteClienteAccount = async (clientId: string) => {
		const clientExists = await db.query.users.findFirst({ where: eq(schema.users.id, clientId) })
